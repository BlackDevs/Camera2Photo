...
class MainActivity : AppCompatActivity(), Executor {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lensFacing = if ("0" == intent.getStringExtra(CAMERA_MODE) ?: "0") {
            CameraSelector.LENS_FACING_BACK
        } else {
            CameraSelector.LENS_FACING_FRONT
        }
        if (false == intent?.getBooleanExtra(CAMERA_ROTATE_ENABLED, true)) {
            rotateCamera?.visibility = View.GONE
        }
        if (false == intent?.getBooleanExtra(CAMERA_FLASH_ENABLED, true)) {
